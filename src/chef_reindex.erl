...
%% @doc Reindex the specified `Index' in `OrgName'. This function avoids use of
%% rabbitmq. Flatten/expand is handled by `chef_index_expand' which also takes care of
%% POSTing the data to solr. When this function returns, all objects in the specified index
%% will have been reindexed and received by solr.
%%
...
-spec ez_reindex_direct(binary(), index()) -> ok.
ez_reindex_direct(OrgName, Index) ->
    DbCtx = make_context(),
    ...
    DoBatch = fun(Batch, _Acc) ->
                      ...
                      send_direct_to_solr(OrgId, Index, Objects, NameIdDict)
              end,
...
send_direct_to_solr(_, _, {error, _} = Error, _) ->
...
send_direct_to_solr(OrgId, Index, Objects, NameIdDict) ->
    ...
    SolrCtx = lists:foldl(
      fun(SO, Ctx) ->
              ...
              chef_index_expand:add_doc(Ctx, Id, IndexEjson, Index, OrgId)
      end, chef_index_expand:init_add_docs(), Objects),
    chef_index_expand:post_to_solr(SolrCtx).
